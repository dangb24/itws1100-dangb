	  	...
	  	$.ajax({
    		...
    		success: function(responseData, status){
      		...
      		$(responseData).find('item').each(function() {
				...
				output += '<div id=\'date\'>' + $(this).find('description').text() + '</div><br />';
      		});
      		output += '</ul>';
      		$('#RSSOutput').html(output);
    		}, error: function(msg) {
      		// there was a problem
      		alert('There was a problem: ' + msg.status + ' ' + msg.statusText);
    		}
 		...
 		$.ajax({
		    ...
		    $(responseData).find('entry').each(function() {
				output += '<li><a href="' + $(this).find('link').text() + '" target="_blank">';
			 	output += $(this).find('title').text();
			 	output += '</a></li>';
			 	output += '<div id=\'date\'>' + $(this).find('updated').text() + '</div><br />';
				output += '<div id=\'date\'>' + $(this).find('summary').text() + '</div><br />';
		    });
		    output += '</ul>';
		    $('#AtomOutput').html(output);
		 	}, error: function(msg) {
		    // there was a problem
		    alert('There was a problem: ' + msg.status + ' ' + msg.statusText);
		 	}
